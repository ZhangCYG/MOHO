...
class SDFNetwork(nn.Module):
    ...
    def forward(self, inputs, get_sdf=False, get_gradient=False):
        if get_gradient:
            return self.gradient(inputs)
        
        inputs = inputs * self.scale
        xyz = inputs[:, :self.xyz_dim]
        latent = inputs[:, self.xyz_dim:]
        ...
        for l in range(0, self.num_layers - 1):
            ...
            if l < self.num_layers - 2:
                x = self.activation(x)
        ret = torch.cat([x[:, :1] / self.scale, x[:, 1:]], dim=-1)
    ...
    def gradient(self, x):
        x_p = x[:, :3].clone().detach()
        x_f = x[:, 3:].clone().detach()
        x_p.requires_grad_(True)
        x_f.requires_grad_(False)
        y = self.sdf((torch.cat([x_p, x_f], dim=-1)))
